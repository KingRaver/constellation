...
- [x] **Upgrade `next` from 14.2.5 → 14.2.35** *(package.json)*
...
- [x] **Add HTTP security headers to `next.config.mjs`**
...
- [x] **Add a `.gitignore`** *(project root)*
...
- [x] **Add touch event support to the canvas** *(components/ConstellationCanvas.tsx)*
...
- [x] **Add Escape key handler to close the panel** *(app/page.tsx)*
...
- [x] **Remove unused `hasSelectedRef` / `hasSelectedNode` prop** *(ConstellationCanvas.tsx:237–238, page.tsx:75)*
...
- [x] **Remove unused `hoveredId` local variable** *(ConstellationCanvas.tsx:575)*
...
- [x] **Derive node/edge counts from data, not a hardcoded string** *(app/page.tsx:108)*
...
- [x] **Add an error boundary around `ConstellationCanvas`** *(app/page.tsx)*
...
- [x] **Add OG/Twitter meta tags to `app/layout.tsx`**
...
- [x] **Add `aria-label` to the canvas element** *(ConstellationCanvas.tsx:681)*
...
- [x] **Cache the `edgeNodeMap` instead of rebuilding it every frame** *(ConstellationCanvas.tsx:344, 381)*
...
- [x] **Call `getConnected` once per frame instead of twice** *(ConstellationCanvas.tsx:593–594)*
...
- [x] **Debounce the resize handler** *(ConstellationCanvas.tsx:648–650)*
...
- [x] **Add a CI workflow** *(.github/workflows/ci.yml)*
